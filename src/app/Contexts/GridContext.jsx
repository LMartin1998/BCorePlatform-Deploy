...
function GridProvider({ children }) {
  ...
  const searchedBlocks = (getIdsWithViewBoxAndPoints, searchValue) => {
    const searchedBlock = getIdsWithViewBoxAndPoints.filter((item) => {
  ...
  const torqueTubeBySections = () => {
    if (jid) {
      ...
      torqueTubeBySections.forEach((element) => {
        ...
        const sectionArray = torqueTube.map(
          (tt) => `${sectionsId}, ${tt.torqueTubeId}`
        );
  ...
  const torqueTubeState = (sectionId, torqueTubeId) => {
    if (jid) {
      ...
      const torqueTube = torqueTubes.find(
        (tt) => tt.torqueTubeId === torqueTubeId
      );
  ...
  const maxPanels = (sectionId) => {
    if (jid) {
      ...
      if (item) {
        const torqueTubeBySections = item.sections || [];
        const matchingSections = torqueTubeBySections.filter(
          (section) => section.sectionId === sectionId
        );
  ...
  const initialStateTT = (sectionId, torqueTubeId) => {
    const readSectionId =
      readtt && readtt[sectionId]
        ? readtt[sectionId][torqueTubeId].split(", ")[0]
        : "s-1";
    const readTorqueTubeId =
      readtt && readtt[sectionId]
        ? readtt[sectionId][torqueTubeId].split(", ")[1]
        : "t-1";
    return {
  ...
  const handleTorqueTubes = (columnIndex, rowIndex) => {
    return TorqueTubesLocalStorage(
      columnIndex,
      rowIndex,
      toolMode,
      initialStateTT(rowIndex, columnIndex)
    );
  };

  const handlePanels = (columnIndex, rowIndex, index) => {
    return PanelsLocalStorage(
      columnIndex,
      rowIndex,
      index,
      toolMode,
      initialStatePanel(rowIndex, columnIndex, index)
    );
  };
